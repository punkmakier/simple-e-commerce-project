<?php
    ...
    $sqlShowPendings = "SELECT * FROM `orderproducts` WHERE `order_status` = 'Pending'";
...
<body>
    ...
    <div class="display-4 p-4 shadow bg-light"><a href="adminPage.php"><i class="fas fa-arrow-left"></i></a> Order Request</div>
    </div>
     
        <form action="searchOrderRequest.php" method="get">
            <div class="input-group" style="width: 300px; float: right; margin-top: 20px; margin-right: 20px;">
                <input name="searchOrderRequest" type="text" class="form-control" placeholder="Search" aria-label="Recipient's username" aria-describedby="button-addon2" required>
  ...
  <tbody>
        <?php if($rowCount = mysqli_num_rows($sqlShowPendings) > 0){
            while($results = mysqli_fetch_array($sqlShowPendings)){?>
    <tr>
