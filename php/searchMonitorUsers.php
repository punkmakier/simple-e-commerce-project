<?php
    ...
    if(isset($_GET['searchUser']))
    {
        ...
        $item = $_GET['searchUser'];

        $querySearch = "SELECT * FROM `useraccounts` WHERE `user_name` = '$item'";
    ...
    if(isset($_POST['searchDeactivateBTN']))
    {
        $sqlDeactivate = "UPDATE `useraccounts` SET `user_status`= 'Deactivate' WHERE `userID` = $_POST[searchdeactiveIDAccount]";
        if($sqlDeactivate = mysqli_query($connection,$sqlDeactivate))
        {
            header("Location: searchMonitorUsers.php");
        }
        else{
    ...
    if(isset($_POST['searchActivateBTN']))
    {
        $sqlActivate = "UPDATE `useraccounts` SET `user_status`= 'Activate' WHERE `userID` = $_POST[searchactivateIDAccount]";
        if($sqlActivate = mysqli_query($connection,$sqlActivate))
        {
            header("Location: searchMonitorUsers.php");
        }
        else{
...
<body>
    ...
    <div class="display-4 p-4 shadow bg-light"><a href="monitorUsers.php"><i class="fas fa-arrow-left"></i> </a> Search Result</div>
    </div>
  ...
  <tbody>
        <?php if($rowCount = mysqli_num_rows($querySearch) > 0){
      ...
      <td><?php echo $results['user_status']; ?></td>
        ...
        <div class="btn-group dropleft">
            ...
            <div class="dropdown-menu p-2">
                ...
                <button type="button" class="btn bg-danger dropdown-item deactiveUser mt-1"><span class=" text-white">Deactive</span></button>
    ...
    <div class="modal-content">
      ...
      <div class="modal-body">
        ...
        <form action="" method="post">
            <input type="hidden" id="activateID" name="searchactivateIDAccount">
            <div class="modal-footer">
        <button type="button" class="btn btn-danger" data-dismiss="modal">Cancel</button>
        <button name="searchActivateBTN" type="submit" class="btn btn-success">Yes! Activate this</button>
      </div>
        </form>
  ...
  <div class="modal-dialog">
    <div class="modal-content">
      ...
      <div class="modal-body">
        ...
        <form action="" method="post">
            <input type="hidden" id="deactiveID" name="searchdeactiveIDAccount">
            <div class="modal-footer">
        <button type="button" class="btn btn-danger" data-dismiss="modal">Cancel</button>
        <button name="searchDeactivateBTN" type="submit" class="btn btn-warning">Yes! Deactive this</button>
      </div>
        </form>
